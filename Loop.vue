        ...
        >
            ...
            >
                ...
                <Loop
                    ...
                    :data-key="dataKey"
                >
                    <template #default="subItem: SlotData">
...
import { traverse, traverseAsync } from './traverse'
        ...
        if (i > -1) {
            if (getChildren(props.modelValue[i]).length === 0) {
                ...
                if (currentExpandState !== toExpandState && div) {
                    ...
                    let subLoopWrapper = (
                        ...
                        Array.from(c.classList).includes('sub-loop-wrapper')
                    )
                    if (subLoopWrapper) {
                        let subLoop = (
                            Array.from(
                                subLoopWrapper.children
                            ) as HTMLDivElement[]
                        ).find((c) =>
                            Array.from(c.classList).includes('sub-loop')
                        )
                        if (subLoop) {
                            if (!toExpandState) {
            ...
            if (parent) {
                return toggleByData(parent, options).then(() => {
                    return subLoopRefs.value?.forEach((r) =>
                        r.toggle(key, options)
                    )
                })
            }
        }
...
const emit = defineEmits<{
    (e: 'update:model-value', val: T[]): void
}>()
